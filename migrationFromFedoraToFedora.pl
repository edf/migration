...
if ( $option =~ m/:/ ) {    # matches a PID
    ...
    my (
        ...
        $modsDatastream,   $dcDatastream,    @viewableByUser,
        @viewableByRole,   @relsExtAccess,   @manageableByUser,
        @manageableByRole
    );
    ...
    foreach my $node ( $nodesetResult->get_nodelist ) {
        ...
        foreach my $line (@answer) {
            if ( $line =~ /BasicCollection/ ) {       #is PID a collection PID?
                ...
                my $listCollectionMembershipSearchString = 'select $member from <#ri> where ($member <fedora-rels-ext:isMemberOf> <info:fedora/'
                  . $collectionPid
